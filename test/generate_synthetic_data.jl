...
<atomic_positions>""",
        ...
        for i = 1:natoms
            ...
            println(
                f,
                "<atom name=\"$(species[i])\" index=\"$(i)\">$(@sprintf("%20.14f %20.14f %20.14f", cart...))</atom>",
            )
        end
        println(
            f,
...
<nbnd>$(nbands)</nbnd>""",
        ...
        for ik = 1:nk
            ...
            @printf(
                f,
                "<k_point weight=\"%.12f\">%20.14f %20.14f %20.14f</k_point>\n",
                w,
                kpoints[1, ik],
                kpoints[2, ik],
                kpoints[3, ik]
...
function write_wien2k(dir, lattice, positions, species, kpoints, ebands, fermi)
    ...
    open(joinpath(dir, "Synthetic.struct"), "w") do f
        ...
        for i = 1:natoms
            ...
            @printf(
                f,
                "%-2s         NPT=  781  R0=0.00010000 RMT=   2.00000   Z:%5.1f\n",
                species[i],
                i == 1 ? 6.0 : 7.0
            )
    ...
    open(joinpath(dir, "Synthetic.energy"), "w") do f
        for ik = 1:nk
            @printf(
                f,
                "%19.12E%19.12E%19.12E%10d%6d%6d  1.0   \n",
                kpoints[1, ik],
                kpoints[2, ik],
                kpoints[3, ik],
                ik,
...
function generate_all(output_base::String)
    ...
    for (name, a, b, c, α, β, γ) in cells
        ...
        # Write all formats
        write_gene(
            joinpath(output_base, "Synthetic.GENE.$name"),
            lattice,
            frac_positions,
            species,
            kpoints,
        ...
        write_vasp(
            joinpath(output_base, "Synthetic.vasp.$name"),
            lattice,
            frac_positions,
            species,
            kpoints,
        ...
        write_qe(
            joinpath(output_base, "Synthetic.ESPRESSO.$name"),
            lattice,
            frac_positions,
            species,
            kpoints,
        ...
        write_wien2k(
            joinpath(output_base, "Synthetic.W2K.$name"),
            lattice,
            frac_positions,
            species,
            kpoints,
        ...
        if write_abinit(
            joinpath(output_base, "Synthetic.abinit.$name"),
            lattice,
            frac_positions,
            species,
            kpoints,
...
function verify_loaders(data_base::String)
    if !BOLTZTRAP_AVAILABLE
        println(
            "BoltzTraP not available. Run with: julia --project=. test/generate_synthetic_data.jl --verify",
        )
        return false
    end
