...
transport_sci =
    run_integrate(interp; temperatures = [300.0], scissor = 1.17, verbose = true)
...
println(
    "    Chemical potential range: $(round(transport_nosci.mu_values[1], digits=4)) .. $(round(transport_nosci.mu_values[end], digits=4)) eV",
)
println("    Number of mu points: $(length(transport_nosci.mu_values))")
println("  With scissor (1.17 eV):")
println(
    "    Chemical potential range: $(round(transport_sci.mu_values[1], digits=4)) .. $(round(transport_sci.mu_values[end], digits=4)) eV",
)
...
p1 = plot(
    mu_nosci,
    transport_nosci.seebeck[1, 1, iT, :] .* 1e6;
    title = "T = 300 K",
    ylabel = "S_xx (μV/K)",
    ylims = (-1100, 1100),
    xlabel = "",
...
plot!(
    p1,
    mu_sci,
    transport_sci.seebeck[1, 1, iT, :] .* 1e6;
    label = "scissor (1.17 eV)",
    linewidth = 2,
    color = :red,
)
p2 = plot(
    mu_nosci,
    transport_nosci.sigma[1, 1, iT, :];
    ylabel = "σ_xx/τ (S/m)",
    yscale = :log10,
    ylims = (1e14, 1e21),
    xlabel = "",
...
plot!(p2, mu_sci, transport_sci.sigma[1, 1, iT, :]; linewidth = 2, color = :red)
p3 = plot(
    mu_nosci,
    transport_nosci.kappa[1, 1, iT, :];
    ylabel = "κ_xx/τ (W/m/K)",
    yscale = :log10,
    ylims = (1e10, 1e16),
    xlabel = "μ - E_F (eV)",
...
plot!(p3, mu_sci, transport_sci.kappa[1, 1, iT, :]; linewidth = 2, color = :red)
p = plot(
    p1,
    p2,
    p3;
