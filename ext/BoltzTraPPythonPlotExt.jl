...
function _plot_bands_on_ax!(
    ...
    for i = 1:nbands
        ax.plot(bpd.kdist, bpd.ebands_eV[i, :]; color, linewidth, linestyle)
    end
...
function _plot_transport_on_ax!(
    ax,
...
function BoltzTraP.savefig_publication(
    bpds::AbstractVector{<:BandPlotData},
    path::AbstractString;
    axis_width_cm = 8.0,
    axis_height_cm = 6.0,
    ...
    if overlay
        ...
        nrows, ncols = layout
        fig_w, fig_h, positions =
            _layout_axes(axis_width_cm, axis_height_cm, length(bpds); nrows, ncols)
        fig = PythonPlot.figure(figsize = (fig_w, fig_h))
        for (i, bpd) in enumerate(bpds)
...
function BoltzTraP.savefig_publication(
    tpds::AbstractVector{<:TransportPlotData},
    path::AbstractString;
    axis_width_cm = 8.0,
    axis_height_cm = 6.0,
    ...
    nrows, ncols = layout
    fig_w, fig_h, positions =
        _layout_axes(axis_width_cm, axis_height_cm, length(tpds); nrows, ncols)
    fig = PythonPlot.figure(figsize = (fig_w, fig_h))
