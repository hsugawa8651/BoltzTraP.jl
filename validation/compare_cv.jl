...
function main()
    ...
    @printf("  epsilon range: %.4f to %.4f Ha\n", minimum(ref["epsilon"]), maximum(ref["epsilon"]))
    ...
    if verbose
        ...
        println("  " * "-" ^ 54)
        @printf("  %6s  %15s  %15s  %12s\n", "mu idx", "Julia [J/K]", "Python [J/K]", "Rel. Diff")
        println("  " * "-" ^ 54)
        T_idx = findfirst(==(300.0), ref["T_range"])
        if !isnothing(T_idx)
            for i in 1:length(ref["mu_range"])
                jl = cv_julia[T_idx, i]
    ...
    mean_rel = sum(abs.(cv_julia - cv_python) ./ max.(abs.(cv_python), 1e-30)) / length(cv_python)
