...
Test end-to-end integrate workflow for PbTe VASP (non-magnetic thermoelectric).
"""
function compare_pbte_integrate_e2e()
    ref_file = joinpath(DATA_DIR, "pbte_end2end.npz")
    if !isfile(ref_file)
        @test_skip "pbte_end2end.npz not found (run generate_5_e2e.py)"
        return
    end

    ref = load_reference("pbte_end2end")

    if !haskey(ref, "eband")
        @test_skip "Transport data not in pbte_end2end.npz (regenerate with Python)"
        return
    end

    @testset "PbTe E2E integrate" begin
        if !@isdefined(BoltzTraP) || !isdefined(BoltzTraP, :calc_onsager_coefficients)
            @test_skip "calc_onsager_coefficients not implemented"
            return
        end
...
Test end-to-end integrate workflow for Li VASP (spin-polarized, dosweight=1).
"""
function compare_li_integrate_e2e()
    ref_file = joinpath(DATA_DIR, "li_end2end.npz")
    if !isfile(ref_file)
        @test_skip "li_end2end.npz not found (run generate_5_e2e.py)"
        return
    end

    ref = load_reference("li_end2end")

    if !haskey(ref, "eband")
        @test_skip "Transport data not in li_end2end.npz (regenerate with Python)"
        return
    end

    @testset "Li VASP E2E integrate" begin
        if !@isdefined(BoltzTraP) || !isdefined(BoltzTraP, :calc_onsager_coefficients)
            @test_skip "calc_onsager_coefficients not implemented"
            return
        end
...
Test end-to-end integrate workflow for Li GENE (from spin-polarized VASP, dosweight=1).
"""
function compare_gene_li_integrate_e2e()
    ref_file = joinpath(DATA_DIR, "gene_li_end2end.npz")
    if !isfile(ref_file)
        @test_skip "gene_li_end2end.npz not found (run generate_5_e2e.py)"
        return
    end

    ref = load_reference("gene_li_end2end")

    if !haskey(ref, "eband")
        @test_skip "Transport data not in gene_li_end2end.npz (regenerate with Python)"
        return
    end

    @testset "Li GENE E2E integrate" begin
        if !@isdefined(BoltzTraP) || !isdefined(BoltzTraP, :calc_onsager_coefficients)
            @test_skip "calc_onsager_coefficients not implemented"
            return
        end
        ...
        @testset "solve_for_mu" begin
            if !haskey(ref, "mu0")
                @test_skip "mu0 not in gene_li_end2end.npz (GENE nelect unavailable)"
                return
            end
...
"""
function compare_bi2te3_integrate_e2e()
    ref_file = joinpath(DATA_DIR, "bi2te3_end2end.npz")
    if !isfile(ref_file)
        @test_skip "bi2te3_end2end.npz not found (run generate_5_e2e.py)"
        return
    end

    ref = load_reference("bi2te3_end2end")

    if !haskey(ref, "eband")
        @test_skip "Transport data not in bi2te3_end2end.npz (regenerate with Python)"
        return
    end

    @testset "Bi2Te3 E2E integrate" begin
        if !@isdefined(BoltzTraP) || !isdefined(BoltzTraP, :calc_onsager_coefficients)
            @test_skip "calc_onsager_coefficients not implemented"
            return
        end
    ...
    if isempty(ARGS)
        ...
        for case in ARGS
            ...
            elseif case == "abinit_si_integrate_e2e"
                compare_abinit_si_integrate_e2e()
            elseif case == "pbte_integrate_e2e"
                compare_pbte_integrate_e2e()
            elseif case == "li_integrate_e2e"
                compare_li_integrate_e2e()
            elseif case == "gene_li_integrate_e2e"
                compare_gene_li_integrate_e2e()
