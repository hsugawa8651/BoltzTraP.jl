...
function run_scaling_benchmark(;
    ...
    for nkpt in nkpt_values
        ...
        for run = 1:n_runs
            ...
            println(
                "  Run $run: $(round(total, digits=1)) ms (interp: $(round(interp, digits=1)), integ: $(round(integ, digits=1)))",
            )
        end
        ...
        println(
            "  Median: $(round(median_total, digits=1)) ms (interp: $(round(median_interp, digits=1)), integ: $(round(median_integ, digits=1)))",
        )
